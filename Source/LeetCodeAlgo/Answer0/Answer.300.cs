...
namespace LeetCodeAlgo
{
    public partial class Answer
    {
        ...
        public IList<IList<int>> PalindromePairs(string[] words)
        {
            ...
            var prefixTree = new TrieItem();//build the root of prefix tree
            for (int i = 0; i < n; i++)
            {
                var curr = prefixTree;
                for (int j = 0; j < words[i].Length; j++)
                {
                    curr.nums.Add(i);//store all indexes which can match the prefix
                    if (!curr.dict.ContainsKey(words[i][j]))
                        curr.dict.Add(words[i][j], new TrieItem());
                    curr = curr.dict[words[i][j]];
                }
                curr.index = i;//store the index of whole word matched
            }
            for (int i = 0; i < n; i++)
            {
                var curr = prefixTree;
                for (int j = words[i].Length - 1; j >= -1; j--)
                {
                    if (j >= 0)
                    {
                        if (curr.index != -1 && curr.index != i)
                        {
                            //pattern contains 3 parts, Palindrome= {prefix} + [0,j] + substring of [j+1,words[i].Length - 1]
                            //eg.  "s", "lls" is a pair = "s" + "ll" + "s" is Palindrome, "ll" is substring of [0,1]
                            if (PalindromePairs_IsValid(words[i], 0, j))
                                res.Add(new List<int> { curr.index, i });//curr.index is the prefix index, whole word match
                        }
                        if (!curr.dict.ContainsKey(words[i][j])) break;//if prefix cannot matched here, just break
                        curr = curr.dict[words[i][j]];
                    }
                    else //using j==-1 to match the whole word
                    {
                        if (curr.index != -1 && curr.index != i)
                            res.Add(new List<int> { curr.index, i });//match whole word, eg. "abcd" and "dcba"
                        //all other words that match the whole word as reversed prefix, and other part is Palindrome!
                        //eg, "abcd" and "edcba", now index of "edcba" is in curr.nums, and other part "e" is a Palindrome
                        foreach (var k in curr.nums)
                        {
                            if (PalindromePairs_IsValid(words[k], words[i].Length, words[k].Length - 1))
                                res.Add(new List<int> { k, i });
                        }
                    }
                }
            }
        ...
        public IList<IList<int>> PalindromePairs_Suffix(string[] words)
        {
            int n = words.Length;
            List<IList<int>> res = new List<IList<int>>();
            var suffixTree = new TrieItem();
            for (int i = 0; i < n; i++)
            {
                var curr = suffixTree;
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for (int j = 0; j <= words[i].Length; j++)
                {
                    if (j < words[i].Length)
                    {
                        if (curr.index != -1 && curr.index != i)
                        {
                            if (PalindromePairs_IsValid(words[i], j, words[i].Length - 1))
                            {
                                res.Add(new List<int> { i, curr.index });
                            }
                        }
                        if (!curr.dict.ContainsKey(words[i][j])) break;
                        curr = curr.dict[words[i][j]];
                    }
                    else
                    {
                        ...
                        foreach (var k in curr.nums)
                        {
                            if (PalindromePairs_IsValid(words[k], 0, words[k].Length - words[i].Length - 1))
                            {
        ...
        private bool PalindromePairs_IsValid(string word, int i, int j)
        {
            while (i < j)
